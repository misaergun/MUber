...
class PushNotificationSystem
{
  ...
  {
    //1. Terminated (uygulama kapalıyken bildirime basarak uygulamayı açma)
    FirebaseMessaging.instance.getInitialMessage().then((RemoteMessage? remoteMessage)
    {
      if(remoteMessage != null)
      {
        //display ride request information - user info who request a ride
        readUserRideRequestInformation(remoteMessage.data["tripID"] , context);
      }
    });

    //2. Foreground(uygulama açıkken bildirimin iletilmesi)
    FirebaseMessaging.onMessage.listen((RemoteMessage? remoteMessage)
    {
      readUserRideRequestInformation(remoteMessage!.data["tripID"] , context);
    });

    //3. Background(uygulama arka planda açıkken bildirime tıklayarak uygulamaya dönme)
